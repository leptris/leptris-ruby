  ...
  module XPath
    ...
    module Conversion
      ...
      def self.to_compatible_types(left, right)
        if left.is_a?(Taurus::NodeSet) || left.respond_to?(:text)
          left = to_string(left)
        end

        if right.is_a?(Taurus::NodeSet) || right.respond_to?(:text)
