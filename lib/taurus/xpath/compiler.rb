...
module Taurus
  module XPath
    ...
    class Compiler
      ...
      def compile(ast)
        ...
        proc_ast = literal(:lambda).add_block(document) do
          ...
          if return_nodeset?(ast)
            # Create NodeSet using send node: Taurus::NodeSet.new([], context)
      ...
      # Create a constant reference like Taurus::Document
      def const_ref(*parts)
      ...
      def on_eq(ast, input, &block)
        conv = literal(Taurus::XPath::Conversion)
      ...
      def on_lt(ast, input, &block)
        conversion = literal(Taurus::XPath::Conversion)
