...
module Taurus
  module XPath
    module Ruby
      ...
      #
      # This class takes a {Taurus::XPath::Ruby::Node} instance and converts it
      # (and its child nodes) to a String that can be passed to `eval`.
      #
      # @private
      class Generator
        # @param [Taurus::XPath::Ruby::Node] ast
        # @return [String]
        def process(ast)
        ...
        # Processes a constant reference node (e.g., Taurus::Document).
        #
        # @param [Taurus::XPath::Ruby::Node] ast
        # @return [String]
        def on_const(ast)
