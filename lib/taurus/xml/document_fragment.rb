...
# taurus_parse_fragment. Children of this fragment are the parsed nodes;
...
class Taurus::XML::DocumentFragment
  ...
  def self.parse(xml, document)
    ...
    raw = Taurus::XML::FFI.taurus_parse_fragment(
      xml.to_s, xml.to_s.bytesize, document.c_ptr, status_ptr)
    if raw.null?
      raise Taurus::XML::ParseError,
        "taurus_parse_fragment failed (status=#{status_ptr.read_int})"
    end
    new(document, raw)
  end

  def children
    count = Taurus::XML::FFI.taurus_element_child_count(@c_ptr)
    nodes = []
    ptr = Taurus::XML::FFI.taurus_node_first_child(@c_ptr)
    until ptr.nil? || ptr.null?
      nodes << Taurus::XML::Node.wrap(ptr, @document)
